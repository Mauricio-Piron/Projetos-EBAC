...
import { priceAdjust } from '../../components/Product'
    ...
    <S.Container className={isOpen ? 'is-open' : ''}>
      ...
      <S.Sidebar>
        <S.PaymentContainer className={paymentStep === 'cart' ? 'visible' : ''}>
          {items.length > 0 ? (
            <>
                ...
                {items.map((item) => {
                  return (
                    <S.CartItem key={item.id}>
                      ...
                      <S.Button onClick={() => removeCart(item.id)}>
                        <img src={lixeira} alt="botão remover" />
                      </S.Button>
                    </S.CartItem>
                  )
                })}
              ...
              <S.Values>
                <p>Valor total</p>
                <p>{priceAdjust(getTotalPrice(items))}</p>
              </S.Values>
              <S.TagButton
                type="button"
        ...
        >
          ...
          >
            <h3>
              Pagamento - Valor a pagar {priceAdjust(getTotalPrice(items))}
            </h3>
            <S.InputGroup>
